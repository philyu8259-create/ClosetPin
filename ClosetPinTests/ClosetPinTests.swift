...
final class ClosetPinTests: XCTestCase {
    ...
    func testPhotoPersistenceCropsCenteredGarmentAndRemovesBedSheetBlock() throws {
        ...
        XCTAssertEqual(result.displayImageData.prefix(8), Data([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]))
        XCTAssertLessThan(displayImage.size.width, sourceImage.size.width * 0.82)
        XCTAssertLessThan(displayImage.size.height, sourceImage.size.height * 0.82)
        XCTAssertLessThan(alphaAt(x: 2, y: 2, in: displayImage), 24)
    }
